...
def create_project(request):
    try:
        ...
        new_project = Project(email=email, project=project_name, description=project_description, share=True,
                              owner=user, thumbnail=thumbnail, screen=screen, density=density, created=created,
                              last_updated=created, uuid=uuid_name)
        new_project.save()
...
def artboards(request):
    try:
        ...
        return render(request, 'artboards.html',
                      {'artboards': artboards, 'user': user, 'project': project_name, 'description': description,
                       'share': share, 'screen': screen, 'density': density, 'created': created,
                       'last_updated': last_updated, 'owner': owner, 'shared_members': shared_members})
    except:
        print sys.exc_info()
        return render(request, 'wrong.html')
...
def index(request):
    try:
        ...
        return render(request, 'index.html',
                      {'annotations': json.dumps(annotations), 'url': params, 'artboard': artboard, 'notes': notes,
                       'project': project})
    except:
        print sys.exc_info()
        return render(request, 'wrong.html')
...
def revisions(request):
    try:
        ...
        project_name = request.session['project']
        revisions = Revision.objects.filter(artboard__project__project__contains=project_name,
                                            artboard__artboard__contains=artboard_name)
